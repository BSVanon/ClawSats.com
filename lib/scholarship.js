...
function mountRoutes(app, { claimsDb, scholarshipRemittances, getPendingClaimEntries }) {
  ...
  // --- Record a manual (off-server) distribution for bookkeeping ---
  app.post('/api/scholarships/record-manual', (req, res) => {
    if (!hasScholarshipDistributeAuth(req)) {
      return res.status(401).json({ error: 'Unauthorized.' });
    }
    ...
    if (!Array.isArray(distributions) || distributions.length === 0) {
      return res.status(400).json({
        error: 'Provide { "distributions": [{ identityKey, satoshis, txid, endpoint? }] }'
      });
    }
    let recorded = 0;
    for (const d of distributions) {
      const sats = Math.floor(Number(d.satoshis));
      if (!d.identityKey || !Number.isFinite(sats) || sats <= 0) continue;
      const entry = {
        identityKey: d.identityKey,
        endpoint: d.endpoint || 'manual',
        satoshis: sats,
        txid: d.txid || 'manual-record',
        status: 'sent',
        distributedAt: d.distributedAt || new Date().toISOString(),
        manual: true
      };
      fund.distributions.push(entry);
      fund.totalDistributed += sats;
      recorded += sats;
    }
    saveFund(fund);
    res.json({
      ...
      totalDistributed: fund.totalDistributed,
      totalDistributions: fund.distributions.length,
      message: `Recorded ${recorded.toLocaleString()} sats of manual distributions.`
    });
  });
}
